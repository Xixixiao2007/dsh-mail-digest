...
export function apply(ctx) {
  ...
  ctx.effect(() => ctx.tools.register(defineTool({
    ...
    description:
      ...
      + '【关键】用户在邮箱里**只看到这段摘要**，看不到你的正文、工具调用和前面对话。'
      + '所以摘要必须自包含：如果你这一轮要向用户提问，把问题**连同它的上下文**'
      + '（针对什么、为什么问、各选项的后果）写进摘要本身，不要只写「是否继续？」'
      + '这类没有前情就无法理解的话；也避免「它」「这个」这种指代不明的词。'
    ...
    parameters: {
      summary: {
        ...
        required: true,
        description:
          '要发进邮件的中文摘要，一段连续文本，详略由结论多少决定。'
          + '必须自包含：用户在邮箱里只看到这段话，看不到你的正文与前情；'
          + '若要提问，把问题连同它的上下文一起写清楚。',
      },
      pendingPermissions: {
        type: 'array',
        description:
